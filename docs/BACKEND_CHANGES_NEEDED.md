...
## ✅ Shipped in the 2026-07-06 backend drop (was: §C, §E, §H, §I, §J, §K)

- **§C — real account deletion**: `POST /api/account/delete/` (permanent, cascading, purges Spaces objects; requires the current password, optionally blacklists a passed `refresh_token`; Apple 5.1.1(v)). **Frontend cutover still needed** — `ProfileView.handleDeleteAccount` still calls the reversible `POST /api/profile/deactivate/` while promising permanent deletion, and the new endpoint requires a typed password (the current UI is a plain confirm Alert). See backend `FRONTEND_WIRING_GUIDE.md` §1.8 for the contract + recommended confirmation UX.
- **§E — 500 on `POST /api/profiles/like/`**: almost certainly fixed by the matching rework — migration 019 fixed a real unique-index/arbiter mismatch of exactly the shape our hypothesis named (it capped sponsors at one profile-like per job; Sentry BACKCHANNEL-MONITORING-M), and migrations 022–025 rebuilt profile likes per-job with a `DO NOTHING` upsert whose arbiter now matches the index predicate exactly. **Needs re-verification** with the originally-failing TestFlight account before closing. The frontend's hold-the-card-on-error handling stays regardless.
- **§H — password-reset email**: both suspected causes fixed — emails are normalized to lowercase at every auth boundary + migration 026 canonicalizes existing rows (case-insensitive unique index), and prod now **hard-fails at startup** if `EMAIL_HOST` is unset instead of silently logging emails to the console. Deliverability (SPF/DKIM/DMARC) remains an env checklist item.
- **§I — server-side company lock for verified sponsors**: `update_sponsor_profile` returns **409** ("Company is locked to your verified work email…") when a verified sponsor changes `company` without also changing `work_email` in the same PATCH. Matches our client-side lock; changing the work email (which resets verification) remains the sanctioned path. No frontend change needed — `handleSaveField` already surfaces API errors as a toast.
- **§J — self-like**: fixed structurally (migration 022's generated `like_type` column + `LIKE_TYPE` filters across every affected query, service-layer self-interaction guards, and an `IsApplicant` role gate on job-like/apply). The client-side self-filter in `MatchesView.tsx` (commit `508f518`) is now redundant — remove it in the match-cutover release (below).
- **§K — unsponsor cache invalidation**: affected applicants' `job_matches`/`liked_jobs`/`received_likes` keys are now invalidated after the unsponsor transaction (user set captured pre-withdrawal).

**New frontend work these unlock (tracked on our side):**
1. **Delete-account cutover** (from §C above) — new confirmation screen with password entry → `POST /api/account/delete/`; keep the existing push-token/local-notification/store-reset cleanup around it.
2. **Match-state cutover** — backend's `docs/FRONTEND_MATCH_CUTOVER.md` (its line refs were verified against this repo on 2026-07-06): read match state from `GET /api/matches/*` instead of the **derived** `STATUS === "MATCHED"` on like-list rows (`MatchesView.tsx` canRefer + job-card branches, `JobsView.tsx` Message-vs-Match CTA), and drop the redundant self-filter (`MatchesView.tsx:801-803`). The derived field is a compatibility shim kept alive only for the currently-shipped app; the backend retires it after this release ships.
3. No action needed for: email case normalization (backend normalizes at the boundary), per-job profile likes (additive; the UI already keys on `LIKE_ID` and shows `JOB_TITLE` per row), autofill-endpoint deprecation (the app never called `/api/autofill/` — verified by grep), sponsor-feed time-decay (transparent).
